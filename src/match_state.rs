...
impl Default for MatchState {
    fn default() -> Self {
        Self {
            ...
            score_home: 0,
            score_away: 0,
            kickoff_team: TeamId::Home,
            opening_kickoff_team: TeamId::Home,
            phase_timer: 0.0,
        }
    }
}
...
pub fn place_kickoff(ball: &mut Ball, players: &mut [Player], _kickoff_team: TeamId) {
    ball.pos = Vec2::ZERO;
    ball.vel = Vec2::ZERO;
    ball.held = false;
    for p in players.iter_mut() {
        p.pos = faceoff_world(p.team, p.id);
        p.vel = Vec2::ZERO;
